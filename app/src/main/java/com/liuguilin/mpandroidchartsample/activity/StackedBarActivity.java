...
public class StackedBarActivity extends BaseActivity implements OnChartValueSelectedListener, View.OnClickListener {
    ...
    private void initView() {
        ...
        leftAxis.setDrawGridLines(false);//y轴不显示网格

        // 改变x标签的位置
        XAxis xLabels = mBarChart.getXAxis();
        xLabels.setAxisMaximum(6f);
        ...
        xLabels.setDrawGridLines(false);//x轴不显示网格
    ...
    private void setData() {
        ...
        for (int i = 0; i < 6; i++) {
            if (i == 1 || i == 3 || i == 5) {
                float mult = (50 + 1);
                int val1 = (int) ((int) (Math.random() * mult) + mult / 3);
                int val2 = (int) ((int) (Math.random() * mult) + mult / 3);
                int val3 = (int) ((int) (Math.random() * mult) + mult / 3);
        ...
        mBarChart.getXAxis().setSpaceMin(mBarChart.getBarData().getBarWidth() / 2f);
        mBarChart.getXAxis().setSpaceMax(mBarChart.getBarData().getBarWidth() / 2f);
